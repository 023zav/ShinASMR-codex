...
assert(index.includes("lod-style-v2/lod-07-tokyo-yard-city.webp"), "Tokyo city plate is not preloaded");
...
assert(render.includes("const plateForBand"), "Authoritative plate-per-band resolver is missing");
assert(render.includes("lod-style-v2"), "Renderer does not use the lod-style-v2 art ladder");
assert(render.includes("coverage:"), "Plates do not declare real line-progress coverage");
assert(render.includes("const sampleRoute"), "Authored rail route sampler is missing");
assert(render.includes("const lineProgress"), "Timetable positions are not mapped to line progress");
assert(render.includes("TRAIN_SPRITE_AXIS"), "Train sprites are not axis-aligned to authored rails");
assert(render.includes("laneSpread"), "Dwelling trains are not spread across platform lanes");
...
assert(render.includes("vehicle-alpha/train-n700s-blue.webp"), "Runtime trains do not use the isometric consist sprites");
